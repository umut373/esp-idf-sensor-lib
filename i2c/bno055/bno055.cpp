...
void BNO055::calibrate() {
    ...
    uint8_t offset_buffer[22];
    read_data(REG_OFFSET, offset_buffer, 22);

    write8(REG_OPR_MODE, this->mode);
    delay(10);

    int16_t* offset_ptr = reinterpret_cast<int16_t*>(&this->offsets);
    size_t length = sizeof(offset_buffer) / sizeof(int16_t);

    for (int i = 0; i < length; i++) {
        offset_ptr[i] = get_short_le(offset_buffer, i << 1);
    }
}

void BNO055::calibrate(const calibration_data_t& calib_data) {
    this->offsets = calib_data;
...
std::vector<uint8_t> BNO055::get_calib_status() {
    std::vector<uint8_t> data_vector;
    uint8_t buffer = read8(REG_CALIB_STAT);

    for (int i = 3; i >= 0; i--) {
        data_vector.push_back((buffer >> (2*i)) & 0x03);
    }
...
bool BNO055::is_fully_calibrated() {
    std::vector<uint8_t> calib_status = get_calib_status();
